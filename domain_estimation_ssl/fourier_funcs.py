...
def input_const_values(im, resize, const_abs=True, const_pha=False, n_random=0, const_value=0):
    """
        input: 
            im: データ拡張対象の画像 (C, W, H)
...
def mix_amp_phase_and_mixup(
    im: np.array, root, resize,
    mix_filenames,
    mix_amp=True, mix_pha=False, mixup=True,
    LAMB = 0.7,  # fourier mix rate
) -> np.array:
