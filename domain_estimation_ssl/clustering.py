...
def clustering_exec(config, feats, dim_red_method, clust_method, dataset, log_dir, nmi_output=False):
    ...
    if nmi_output:
        ...
        class_accuracy = accuracy_score(dataset.labels, class_cluster)
        ...
        with open(os.path.join(log_dir, "class_accuracy.txt"), "w") as f:
            f.write(f'class_accuracy:{class_accuracy}\n')
        print(f'nmi:{nmi}')
